...
import { charactersUrl, episodeUrl, locationsUrl } from '../../src/helpers/utils/config';
import CharacterResponse from '../../src/model/ApiModels/characterResponse';
import Character from '../../src/model/DataModel/character';
import CharacterDetails from '../../src/model/DataModel/characterDetail';
import Episode from '../../src/model/DataModel/episode';
import Location from '../../src/model/DataModel/location';
import { User } from '../../src/model/DataModel/user';
import UserServicesStub from './userServiceStub';

@injectable()
export class CharacterServiceStub {
  constructor( @inject(TYPES.AuthService) private userService: UserServicesStub) { }
    ...
    .then(async (response) => {
        ...
        if(user) {
          return new CharacterResponse(
            ...
            page || 1,
            this.getCharacters(responseCharacters, user as User)
          );
        }
  ...
  findById = async (userId: string, id: number): Promise<CharacterDetails> => {
    ...
    return axios.get(`${charactersUrl}/${id}`)
      .then(async (response: any) => {
        const user = await this.userService.getById(userId);
        if(user) {
          characterDetail.character = this.getCharacters([response.data], user as User)[0];
        } else {
          throw new Error('Invalid user');
        }
